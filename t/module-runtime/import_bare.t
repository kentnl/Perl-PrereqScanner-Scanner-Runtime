...
my $prereqs  = $scanner->scan_string($imported_bare_code)->as_string_hash;
my $hash_bad = undef;
for my $key ( 1 .. 5 ) {
  ok( exists $prereqs->{"Example$key"}, "Import bare code required $key" ) or $hash_bad = 1;
}
for my $key ( 3, 5 ) {
  next unless exists $prereqs->{"Example$key"};
  unlike( $prereqs->{"Example$key"}, qr/^0$/, "Import bare code required $key @ some version" ) or $hash_bad = 1;
}
if ($hash_bad) {
